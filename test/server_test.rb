...
class ServerTest < Minitest::Test
  ...
  def test_initialized_recovers_from_indexing_failures
    ...
    stdout, _stderr = capture_subprocess_io do
      ...
      # We need to sleep here to give enough time for indexing to finish, since we have no handles to wait on the thread
      sleep(5)
    end

    assert_match("Error while indexing: boom!", stdout)
  end

  def test_rubocop_errors_push_window_notification
    RubyLsp::Requests::Formatting.any_instance.expects(:perform).raises(StandardError, "boom").once
    ...
    notification = @server.pop_response while notification.message != "window/showMessage"
